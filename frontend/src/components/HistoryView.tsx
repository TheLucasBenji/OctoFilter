  ...
  return (
    <div className="history-view">
      <div className="history-view-header">
        ...
        <span className="history-view-count">{items.length} entrada{items.length !== 1 ? 's' : ''}</span>
      </div>
          ...
          <tbody>
            {items.map(item => (
              <tr key={item.id} className="history-row">
                ...
                <td className="history-filter">
                  {FILTER_LABELS[item.filter_type] ?? item.filter_type}
                </td>
                ...
                <td className="history-actions">
                  ...
                  <button
                    className="hist-btn hist-btn-load"
                    onClick={() => handleLoadConfigById(item.id)}
                  >
                    Cargar config
                  </button>
                  <button
